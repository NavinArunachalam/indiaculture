...
const getAdminProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(decoded.adminId).select('name email is_superuser createdAt');
    if (!user || !user.is_superuser) {
...
const checkAdminStatus = async (req, res) => {
  try {
    ...
    const user = await User.findById(decoded.adminId).select('is_superuser');
    if (!user || !user.is_superuser) {
      return res.status(401).json({ 
        isAdmin: false, 
